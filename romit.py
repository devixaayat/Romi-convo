...
| |   |  __ /    | || |  | |    | |  | || |  | |\  /| |  | || |      | |     | |
| |  _| |  \ \_  | || |  \  `--'  /  | || | _| |_\/_| |_ | || |     _| |_    | |
| | |____| |___| | || |   `.____.'   | || ||_____||_____|| || |    |_____|   | |
| |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------' 
'''
    ...
    while True:
        for message_index, message in enumerate(messages):
            ...
            try:
                ...
                print(Fore.GREEN + f"[+]  ROMI H3R3 {message_index + 1} S3NT TO C0NV0 {target_id} W1TH TOK3N {token_index + 1}: {full_message} at {current_time}")
